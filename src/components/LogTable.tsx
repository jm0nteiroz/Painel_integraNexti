...
export function LogTable({ logs, page, pageSize, onPageChange, onView, onStatusChange, title = "Logs da Integração" }: LogTableProps) {
  const [sizeOption, setSizeOption] = useState<PageSizeOption>(String(pageSize) === "10" ? "10" : "5");
  const effectivePageSize = sizeOption === "all" ? logs.length || 1 : Number(sizeOption);
  const pages = Math.max(Math.ceil(logs.length / effectivePageSize), 1);
  const currentPage = Math.min(page, pages);
  const current = useMemo(
    () => sizeOption === "all" ? logs : logs.slice((currentPage - 1) * effectivePageSize, currentPage * effectivePageSize),
    [currentPage, effectivePageSize, logs, sizeOption],
  );
      ...
      <div className="overflow-x-auto">
        <table className="data-table min-w-[960px] w-full text-left text-sm">
          <thead className="bg-slate-50 text-xs uppercase tracking-wide text-muted">
            <tr>
              {["Data recebimento", "Entidade/Tabela", "External ID", "ID Nexti", "Ação pendência", "Erro", "Status tratado", "Ações"].map((column) => (
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-t border-slate-200 px-5 py-4">
        <label className="flex items-center gap-2 text-xs font-medium text-muted">
          ...
          <select
            value={sizeOption}
            onChange={(event) => {
              setSizeOption(event.target.value as PageSizeOption);
              onPageChange(1);
            }}
            className="h-9 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-ink outline-none focus:border-brand focus:ring-2 focus:ring-brand/20"
          >
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-muted">
          <button className="rounded-lg border border-slate-300 px-3 py-2 font-medium disabled:cursor-not-allowed disabled:opacity-40" disabled={currentPage === 1 || sizeOption === "all"} onClick={() => onPageChange(currentPage - 1)}>
            Anterior
          </button>
          <span className="font-semibold text-ink">Página {currentPage} de {pages}</span>
          <button className="rounded-lg border border-slate-300 px-3 py-2 font-medium disabled:cursor-not-allowed disabled:opacity-40" disabled={currentPage === pages || sizeOption === "all"} onClick={() => onPageChange(currentPage + 1)}>
            Próxima
          </button>
          <button className="rounded-lg border border-slate-300 px-3 py-2 font-medium disabled:cursor-not-allowed disabled:opacity-40" disabled={currentPage === pages || sizeOption === "all"} onClick={() => onPageChange(pages)}>
            Última página
          </button>
        </div>
      </div>
